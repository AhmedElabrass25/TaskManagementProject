...
export default function TaskDetailsModal({ task, onClose }: { task: ITask, onClose: () => void }) {
    const { isOpen, selectedTaskId } = useSelector((state: RootState) => state.taskModal);
    const dispatch = useDispatch();
    const { projectId } = useParams();
    let task: ITask;
    useEffect(() => {
        const getTask = async() => {
            const task=await getSingleTask(projectId as string , task.id as string);
        }
        getTask();
    })
      ...
      <div className="bg-white w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-[1.8] p-8 overflow-y-auto border-r border-slate-100">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex items-center gap-1.5 text-slate-400 text-[12px] font-medium">
                <Image src="/icons/layericon.svg" alt="epic icon" width={12} height={12.7}/>
          ...
          <div className="flex-1 bg-slate-50/50 p-8 space-y-8 overflow-y-auto">
            ...
            <div className="space-y-3">
              <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Status</h3>
            ...
            {/* Assignee Card */}
            <div className="space-y-3">
              <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Assignee</h3>
              <div className="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl shadow-sm">
                <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 border border-white ring-1 ring-slate-100">
                  {task.assignee?.name.split(' ').map(n => n[0]).join('') || "UN"}
                </div>
                <div>
                  <p className="text-[13px] font-bold text-slate-800">{task.assignee?.name || "Unassigned"}</p>
                  <p className="text-[11px] text-slate-400 font-medium">{task.assignee?.department || "Member"}</p>
                </div>
              </div>
            </div>
            ...
            {/* Reporter */}
            <div className="space-y-3">
              <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Reporter</h3>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-[10px] font-bold text-indigo-600">
                   {task.created_by.name.split(' ').map(n => n[0]).join('')}
                </div>
                <p className="text-[13px] font-semibold text-slate-700">{task.created_by.name}</p>
              </div>
            </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <button className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 text-sm font-semibold transition-colors px-4 py-2">
            <Image src="/icons/copyicon.svg" alt="link icon" width={15} height={7.5} />
            Copy link
          </button>
          <button 
            onClick={onClose}
